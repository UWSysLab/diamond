...
def getCommandFunc(workingDir, configFile, keyFile):
    return workingDir + "/docc --config " + workingDir + configFile + " --keys " + workingDir + keyFile + " --numkeys " + NUM_KEYS + " --time " + NUM_SECONDS + " --increment"
...
experiment_common.runProcesses(getCommandFunc, NUM_CLIENTS, "docc-increment-out")
