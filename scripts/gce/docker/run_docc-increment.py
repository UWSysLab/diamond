...
NUM_KEYS = "100"
NUM_SECONDS = "120"
OUTPUT_DEST = "scripts/experiments/docc-increment/"
NUM_CLIENTS = 10
NUM_FRONTENDS = 10
USE_REDIS = True
...
for i in range(0, NUM_CLIENTS):
    outputFile = "docc-increment-out-" + repr(random.randint(0, sys.maxint))
    configFile = CONFIG_PREFIX + repr(i % NUM_FRONTENDS)
    cmd = WORKING_DIR + "/docc --config " + WORKING_DIR + configFile + " --keys " + WORKING_DIR + KEY_FILE + " --numkeys " + NUM_KEYS + " --time " + NUM_SECONDS + " --increment > " + WORKING_DIR + outputFile
