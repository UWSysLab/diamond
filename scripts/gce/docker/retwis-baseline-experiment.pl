...
my %killDiamondCmd;
my %killBaselineCmd;
$startDiamondCmd{"georeplicated"} = "ssh -t $GCE_IP 'cd diamond-src/scripts; ./manage-servers.py start ../platform/test/gce --keys experiments/keys.txt --numkeys 100000 --batch 64' >> $log 2>&1";
$startDiamondCmd{"local"} = "ssh -t $GCE_IP 'cd diamond-src/scripts; ./manage-servers.py start ../platform/test/gcelocalfiveshards --keys experiments/keys.txt --numkeys 100000 --batch 64' >> $log 2>&1";
