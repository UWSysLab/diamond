...
./desktop-chat-wrapper.sh timed 5 0 transaction verbose $SERVER writer latencyroom stale 100 > $DIR/transaction-writer.log 2> $DIR/transaction-writer.error &
PID1=$!
./desktop-chat-wrapper.sh timed 5 1.0 transaction verbose $SERVER reader latencyroom stale 100 > $DIR/transaction-reader.log 2> $DIR/transaction-reader.error &
PID2=$!
wait $PID1 $PID2

echo Atomic

./desktop-chat-wrapper.sh timed 5 0 atomic verbose $SERVER writer latencyroom nostale 0 > $DIR/atomic-writer.log 2> $DIR/atomic-writer.error &
PID1=$!
./desktop-chat-wrapper.sh timed 5 1.0 atomic verbose $SERVER reader latencyroom nostale 0 > $DIR/atomic-reader.log 2> $DIR/atomic-reader.error &
PID2=$!
wait $PID1 $PID2
...
./baseline-chat-client-wrapper.sh timed 5 0 verbose localhost writer > $DIR/baseline-writer.log 2> $DIR/baseline-writer.error &
PID1=$!
./baseline-chat-client-wrapper.sh timed 5 1.0 verbose localhost reader > $DIR/baseline-reader.log 2> $DIR/baseline-reader.error &
PID2=$!
wait $PID1 $PID2
kill $PID_SERVER
